#!/usr/bin/env python
"""post_process.py — Convert gridded emissions to NetCDF for CarbonTracker.

For the v2026b NRT extension this script is run twice, once per
``ff_country.py`` method:

    python post_process.py --method assumed
    python post_process.py --method cm_yearly

Input :  outputs/ff_monthly_2026b_<method>_py.npz   (from ff_country.py)
...
def main(method: CMMethod = "assumed") -> None:
    ...
    # ------------------------------------------------------------------
    # 1. Load Python .npz (produced by ff_country.py)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    print(f"\nRunning split_ct.py --method {method} for CT-format output ...")
    script_dir = os.path.dirname(os.path.abspath(__file__))
    subprocess.check_call(
        [sys.executable, os.path.join(script_dir, "split_ct.py"),
         "--method", method],
        cwd=script_dir,
    )
