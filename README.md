...
| 1 | `ingest_2026b.py` | Read all raw inputs, harmonise 189 country names, regrid EDGAR 0.1° → 1°, compute EI/USGS ratios |
| 2 | `ff_country_2026b.py` | Assign national emissions to grid cells, extrapolate, apply seasonal cycles |
| 3 | `post_process_2026b.py` | Convert Gg C → mol/m²/s, write per-year + monolithic netCDFs, call `split_ct_2026b.py` |
| 3b | `split_ct_2026b.py` | Reformat to CarbonTracker conventions (auto-called by step 3) |
| 4 | `verify_2026b.ipynb` | 90+ automated quality checks across 11 sections |

<details>
<summary><strong>Verification checks (90+ in 11 sections)</strong></summary>

The verification notebook (`verify_2026b.ipynb`) runs 90+ checks organised into two parts:
...
1. **Get new data** — EI xlsx, USGS cement CSV, check for CDIAC/EDGAR updates
2. **Copy scripts** — `cp *_2026b.py *_2027.py`
3. **Update year constants:**
   - `ingest`: `LAST_EI_YEAR`, `EI_XLSX` filename
   - `ff_country`: `yr_ei`, `yr_final`
   - `post_process` / `split_ct`: `NPZ_FILE`, `MONOLITHIC`, `FILE_PREFIX`, `YR3`
4. **Run the pipeline** (see Quick Start)
5. **Check results** — run `verify_2026b.ipynb` (update file paths in setup cell)
...
miller-ff/
├── README.md
├── extrapolate_edgar.py           # Step 0
├── ingest_2026b.py                 # Step 1
├── ff_country_2026b.py             # Step 2
├── post_process_2026b.py           # Step 3
├── split_ct_2026b.py               # Step 3b
├── verify_2026b.ipynb              # Step 4: quality checks
...
./
├── README.md                       ← this file
├── split_ct_2026b.py                ← script that produced the CT-format files below
...
**`ingest_2026b.py`** — The main data-preparation script. Reads all raw inputs, harmonizes country names, computes fractional change ratios, regrids EDGAR 0.1° → 1°, and writes processed intermediates. Takes ~5 minutes (EDGAR regridding is the bottleneck).

```bash
python ingest_2026b.py
```
...
**`verify_2026b.ipynb`** — 90+ checks in 11 sections, split into two parts:
...
- **1k.** CDIAC sector-sum integrity (processed data) — strict check that every row in the processed `CDIAC_national_2020.csv` has |sector_sum − total| ≤ 1 Gg C; verifies that `ingest_2026b.py`'s total-recomputation fix covered all cases. Complements check 1a (which only flags large emitters at 5%).
...
```bash
cp ingest_2026b.py ingest_2027.py
cp ff_country_2026b.py ff_country_2027.py
cp post_process_2026b.py post_process_2027.py
cp split_ct_2026b.py split_ct_2027.py
```
...
# Step 1:
python ingest_2027.py

# Step 2:
python ff_country_2027.py

# Step 3: Post-process to per-year NetCDFs (auto-calls split_ct):
python post_process_2027.py

# Step 4: Run verify_2026b.ipynb (update file paths in the Configuration & Setup section)
```
