;pro ff_country_new2024a, dev=dev, season=season
...
;time ranges
yr1=1992
yr2=2020    ;final year of cdiac data
yrbp=2023   ;final year of BP data
yr3=2024    ;final full year for extrapolated emissions
bpyears=yrbp - yr2   ;percent increases for 2020, 2021, 2022 (2023 no data available, so use guess of 1.00 (0% or flat), which has been roughly true globally for 2015 - 2017)
...
aa=where(cdiactime ge yr1 and cdiactime le yr2)
globtot=cdiacff[*,aa]
...
ncountry2=countdd
ccg_fwrite,file='/Users/john/EDGAR/CDIAC_historical/national/cdiac_countries_1992_'+strtrim(yr2,2)+'.txt',nc=1,countrystr2
...
file='/Users/john/EDGAR/bp/bp_oil_1966_'+strtrim(yrbp,2)+'_frac.csv'
...
fuel=['coal','oil','gas']     ;the order of this array should align with the ordering of the categories in CDIAC national and global files
nfuel=fuel.length
bparr=fltarr(bpyears,ncountry2,nfuel)

;ratios for coal, oil, and gas
for i=0,nfuel-1 do begin
  file='/Users/john/EDGAR/bp/bp_'+fuel[i]+'_1966_'+strtrim(yrbp,2)+'_frac.csv'
...
cem_flare_yr=[2020,2021,2022,2023]
...
cem = [4.2e6, 4.4e6, 4.1e6, 4.1e6]
ceminc=cem/shift(cem,1)

;flaring in billions of cubic m from BP
flare = [148.8,152.7,146.8, 157.1]

flareinc=flare/shift(flare,1)
frac_inc_c=ceminc[1:*]
frac_inc_f=flareinc[1:*]

addarrbp=fltarr(bpyears+1,ncountry2,6)
addarrbp[0,*,*]=countryarr[-1,*,*]

frac_arr = fltarr(bpyears,ncountry2,5)
frac_arr[*,*,0:2] = bparr
frac_arr[*,*,3] = rebin(frac_inc_c,3,ncountry2)
frac_arr[*,*,4] = rebin(frac_inc_f,3,ncountry2)
...
;;1. extrapolate for coal, oil, and gas (indices 1:3)
;for i=0,bpyears-1 do begin
;  addarrbp[i+1,*,1:3]=addarrbp[i,*,1:3]*bparr[i,*,*]
;  addarrbp[i+1,*,4]=addarrbp[i,*,4]*frac_inc_c[i]       ;cement
;  addarrbp[i+1,*,5]=addarrbp[i,*,5]*frac_inc_f[i]       ;flaring
;endfor

;now total is sum of all categories
addarrbp[1:-1,*,0]=total(addarrbp[1:-1,*,1:5],3)

addarr = fltarr(bpyears+1,ncountry2,6)
...
;;write out extrapolated USA for use by other programs
;usaidx=where(countrystr2 eq 'UNITED STATES OF AMERICA')   
;usaarr=reform(countryarr2[*,usaidx,*])/1.e6
;outfile='/Users/john/idl/co2/cdiacff_usa'+strtrim(yr3,2)+'_extrap.txt'
;
;ccg_fwrite,file=outfile,nc=7,findgen(nyears3)+0.5+yr1,$
;  usaarr[*,0],$
;  usaarr[*,1],$
;  usaarr[*,2],$
;  usaarr[*,3],$
...
;  usaarr[*,5]
;
;;write out extrapolated China for use by other programs
;chnidx=where(strmid(countrystr2,0,5) eq 'CHINA')
;chinaarr=reform(countryarr2[*,chnidx,*])/1.e6
;outfile='/Users/john/idl/co2/cdiacff_china'+strtrim(yr3,2)+'_extrap.txt'
;
;ccg_fwrite,file=outfile,nc=7,findgen(nyears3)+0.5+yr1,$
;  chinaarr[*,0],$
;  chinaarr[*,1],$
...
outfile2='/Users/john/idl/co2/cdiacff_all'+strtrim(yr3,2)+'_extrap.txt'
...
;file1='/Users/john/EDGAR/COUNTRY1X1.CODE.mod2.csv'
;This has Western Sahara and Netherland Antilles deleted
file1='/Users/john/EDGAR/COUNTRY1X1.CODE.mod2.2013.csv'
ccg_read,file=file1,delimiter=',',codes

file2='/Users/john/EDGAR/COUNTRY1X1.1993.mod.txt'
...
edgardir = '/Users/john/EDGAR/edgarv8/TOTALS_flx_nc/'
edgarfile = 'fracarr'+'_'+strtrim(yr1,2)+'_'+strtrim(yr3,2)+'.sav'
restore,edgardir+edgarfile                                                ;global EDGAR pixel fractions (at 1x1) from yr1 through yr3
...
for ii=0,nyears3-1 do begin
  ...
  for i=0,ncountry2-1 do begin
    ...
    ;   print,i,countrystr2[i],fractot,ncells
...
addtot[0,1:3]=globtot[nyears1-1,1:3]*bpglobinc[0,*]     ;2021
addtot[1,1:3]=addtot[0,1:3]*bpglobinc[1,*]              ;2022
addtot[2,1:3]=addtot[1,1:3]*bpglobinc[2,*]              ;2023
addtot[3,1:3]=addtot[2,1:3]*1.00                        ;2024

;now cement and flaring
;first cement
addtot[0,5]=globtot[nyears1-1,5]*frac_inc_c[0]     ;2021
addtot[1,5]=addtot[0,5]*frac_inc_c[1]              ;2022
addtot[2,5]=addtot[1,5]*frac_inc_c[2]              ;2023
addtot[3,5]=addtot[2,5]*1.00                       ;2024

;then flaring
addtot[0,4]=globtot[nyears1-1,4]*frac_inc_f[0]     ;2021
addtot[1,4]=addtot[0,4]*frac_inc_f[1]              ;2022
addtot[2,4]=addtot[1,4]*frac_inc_f[2]              ;2023
addtot[3,4]=addtot[2,4]*1.00                       ;2024
...
if keyword_set(season) then begin
    ...
    ccg_fread,file='/Users/john/idl/co2c13/plot2d/emis_mon_usatotal_2col.txt',nc=2,monthff
    ...
    if season eq 'nam' then begin
        lonidxmin=40 & lonidxmax=120 & latidxmin=120 & latidxmax=150
    endif else if season eq 'glb' then begin
        lonidxmin=0 & lonidxmax=359 & latidxmin=0 &latidxmax=179
    ...
    ;regionnb=fluxarr3nb[*,lonidxmin:lonidxmax,latidxmin:latidxmax]
    region_seas=temporary(region)*(seasff3+1)
    ;region_seasnb=temporary(regionnb)*(temporary(seasff3)+1)
    fluxarr3[*,lonidxmin:lonidxmax,latidxmin:latidxmax]=temporary(region_seas)
    ;fluxarr3nb[*,lonidxmin:lonidxmax,latidxmin:latidxmax]=temporary(region_seasnb)
    ...
    if seas2 eq 'euras' and season ne 'glb' then begin
        ...
        ccg_fread,file='/Users/john/EDGAR/eurasian_seasff.txt',skip=3,nc=1,seasffa
        seasff2a=matrix_repeat(seasffa,nyears3,/vector,/swap) ;repeats average sc over nyears2-1
        lonidxmin=160 & lonidxmax=350 & latidxmin=120 & latidxmax=150
        nx=lonidxmax-lonidxmin+1
        ny=latidxmax-latidxmin+1
        ...
        ;region_seasnb=temporary(regionnb)*(temporary(seasff3a))
        fluxarr3[*,lonidxmin:lonidxmax,latidxmin:latidxmax]=temporary(region_seas)
        ;fluxarr3nb[*,lonidxmin:lonidxmax,latidxmin:latidxmax]=temporary(region_seasnb)
    endif
endif

;store fluxarr3 and fluxarr3nb as .sav files
save,fluxarr3,filename='/Users/john/idl/co2/fluxarr3.sav'
;save,fluxarr3isof,filename='/Users/john/idl/co2/fluxarr3isof.sav'
;save,fluxarr3nb,filename='/Users/john/idl/co2/fluxarr3nb.sav'


;;re-calculate annual country totals from grid and compare to originals
;fluxarr4=fltarr(nyears3,ncountry2)       
;fluxarr5=fltarr(nyears3,ncountry2)
;;fluxarr4nb=fltarr(nyears3,ncountry2)       
;eetot=[0]
;for i=0,ncountry2-1 do begin
;    ;for four cases below use subdivision codes, otherwise, not.
;    if (codes[i].field1/100 eq 41 or $      ;Czech
...
;    ;idx=array_indices(temp,ee)
;    print,i,countrystr2[i],fractot,ncells
;    for j=0,nyears3-1 do begin
;        temp2=total(fluxarr3[j*12:j*12+11,*,*],1)/12.
;        ;temp2nb=total(fluxarr3nb[j*12:j*12+11,*,*],1)/12.
;        ;temp3=total(fluxarr[j,*,*],1)
;        temp3=reform(fluxarr1nb[j,*,*])
;        fluxarr4[j,i]=total(temp2[ee])
;        fluxarr5[j,i]=total(temp3[ee])
;        ;fluxarr4nb[j,i]=total(temp2nb[ee])
;    endfor
;    eetot=[eetot,ee]
;endfor

;convert from million tons to PgC
fluxarr3=fluxarr3/1.d6
;covert to PgC per mil
;fluxarr3isof = fluxarrisof/1.d6

;;avoid divide by zero or very small numbers
;aa = where(fluxarr3 gt 1d-10)
;bb = where(fluxarr3 le 1d-10 and fluxarr3 ne 0.D)
;delarr3 = fluxarr3*0.D

;delarr3[aa] = fluxarr3isof[aa]/fluxarr3[aa]
;delarr3[bb] = -30.
;delarr3[bb] = total(fluxarr3isof[aa])/total(fluxarr3[aa])

;write out fluxarr3 into hdf files for tm5
;start at 1992+1/24 and end (yr3+1)-1/24
idate=[1992,1,1,0,0,0]
;sytem time
r=ccg_sysdate()
stop

idate2 = [2000,1,1,0,0,0]
for i=12*8,9*12-1 do begin
  file='/Users/john/EDGAR/country/ct_20240718/co2ff.country_cdiac_bp.seas'+season+seas2+'.1x1.hdf'+chardate(idate2,cut=4)
  tag='ff_country_new2024a.pro'+r.s3+'_'+strtrim((i mod 12)+1,2)
  write_hdf,file,tag,reform(fluxarr3[i,*,*]),append=((i mod 12) ne 0),idate=idate,unit='GtC/yr'
  increase_time,idate,1,/month
endfor

;for i=0,(nyears3)*12-1 do begin
;    file='/Users/john/EDGAR/country/ct_20240718/co2ff.country_cdiac_bp.seas'+season+seas2+'.1x1.hdf'+chardate(idate,cut=4)
;    tag='ff_country_new2024a.pro'+r.s3+'_'+strtrim((i mod 12)+1,2)
;    write_hdf,file,tag,reform(fluxarr3[i,*,*]),append=((i mod 12) ne 0),idate=idate,unit='GtC/yr'
;    increase_time,idate,1,/month
;endfor
