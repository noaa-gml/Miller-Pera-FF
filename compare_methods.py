#!/usr/bin/env python3
"""compare_methods.py — Side-by-side comparison of v2026b methods.
...
def main() -> None:
    if not ASSUMED_PATH.exists():
        msg = f"Missing {ASSUMED_PATH}; run ff_country.py + post_process.py --method assumed first"
        raise FileNotFoundError(msg)
    if not CM_PATH.exists():
        msg = f"Missing {CM_PATH}; run ff_country.py + post_process.py --method cm_yearly first"
        raise FileNotFoundError(msg)
